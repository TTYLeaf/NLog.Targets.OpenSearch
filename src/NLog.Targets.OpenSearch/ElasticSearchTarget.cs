...
{
    /// <summary>
    /// NLog Target for writing to OpenSearch using low level client
    /// </summary>
    [Target("OpenSearch")]
    public class OpenSearchTarget : TargetWithLayout, IOpenSearchTarget
    {
        private IOpenSearchLowLevelClient _client;
        ...
        /// <summary>
        /// Gets or sets the opensearch uri, can be multiple comma separated.
        /// </summary>
        public string Uri
        {
        ...
        /// <summary>
        /// <inheritdoc cref="IOpenSearchTarget.CloudId"/>
        /// </summary>
        ...
        /// <summary>
        /// Set it to true if OpenSearch uses BasicAuth
        /// </summary>
        [Obsolete]
        ...
        /// <summary>
        /// Gets or sets the name of the OpenSearch index to write to.
        /// </summary>
        [RequiredParameter]
        ...
        /// <summary>
        /// Gets or sets a comma separated list of excluded properties when setting <see cref="IOpenSearchTarget.IncludeAllProperties"/>
        /// </summary>
        public string ExcludedProperties { get; set; }

        /// <summary>
        /// Gets or sets the document type for the OpenSearch index.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets a list of additional fields to add to the OpenSearch document.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets an alternative serializer for the OpenSearch client to use.
        /// </summary>
        public IOpenSearchSerializer OpenSearchSerializer { get; set; }

        /// <summary>
        /// Gets or sets if exceptions will be rethrown.
        ///
        /// Set it to true if OpenSearchTarget target is used within FallbackGroup target (https://github.com/NLog/NLog/wiki/FallbackGroup-target).
        /// </summary>
        ...
        protected override void InitializeTarget()
        {
            ...
            else
            {
                ...
                connectionPool = nodes.Count() > 1? (IConnectionPool) new StaticConnectionPool(nodes): (IConnectionPool) new SingleNodeConnectionPool(new Uri(uri));
            }            
            var config = OpenSearchSerializer == null 
                   ? new ConnectionConfiguration(connectionPool)
               : new ConnectionConfiguration(connectionPool, OpenSearchSerializer);
            ...
            if (EnableJsonLayout)
            {
                ...
                {
                    InternalLogger.Info("OpenSearch: Layout-property has type SimpleLayout, instead of the expected JsonLayout");
                }
        ...
        private void SendBatch(ICollection<AsyncLogEventInfo> logEvents)
        {
            try
            {
                ...
                {
                    InternalLogger.Error($"OpenSearch: Server error: {result.ServerError}");
                }

                foreach (var itemWithError in result.ItemsWithErrors)
                {
                    InternalLogger.Error($"OpenSearch: Bulk item failed: index:{itemWithError.Index} result:{itemWithError.Result} type:{itemWithError.Type} error:{itemWithError.Error}");
                }

                if (exception != null)
                {
                    InternalLogger.Error(exception.FlattenToActualException(), $"OpenSearch: Failed to send log messages. Status={result.ApiCall?.HttpStatusCode} Uri={result.ApiCall?.Uri} DebugInformation={result.DebugInformation}");
                }
                else if (InternalLogger.IsTraceEnabled)
                {
                    InternalLogger.Trace("OpenSearch: Send Log DebugInfo={0}", result.DebugInformation);
                }
                else if (InternalLogger.IsDebugEnabled)
                {
                    ...
                    if (warnings != null && warnings.Any())
                    {
                        ...
                        InternalLogger.Debug("OpenSearch: Send Log Warnings={0}", warningInfo);
                    }
                }
            ...
            {
                InternalLogger.Error(ex.FlattenToActualException(), "OpenSearch: Error while sending log messages");
                foreach (var ev in logEvents)
                {
                    ev.Continuation(ex);
                }
        ...
        private Dictionary<string, object> GenerateDocumentProperties(LogEventInfo logEvent)
        {
            ...
            foreach (var field in Fields)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    InternalLogger.Warn(ex, "OpenSearch: Error while formatting field: {0}", field.Name);
                }
            }
        ...
        private object FormatValueSafe(object value, string propertyName)
        {
            ...
            catch (Exception ex)
            {
                ...
                InternalLogger.Debug(ex, "OpenSearch: Error while formatting property: {0}", propertyName);
                return null;
            }
        }
        ...
        private static JsonSerializerSettings CreateJsonSerializerSettings(bool specialPropertyResolver, IList<ObjectTypeConvert> objectTypeConverters)
        {
            ...
            foreach (var typeConverter in objectTypeConverters ?? Array.Empty<ObjectTypeConvert>())
            {
                ...
                if (jsonConverter != null)
                    ...
                    InternalLogger.Debug("OpenSearch: TypeConverter for {0} has no JsonConverter", typeConverter.ObjectType);
            }
            jsonSerializerSettings.Error = (sender, args) =>
            {
                InternalLogger.Debug(args.ErrorContext.Error, $"OpenSearch: Error serializing exception property '{args.ErrorContext.Member}', property ignored");
